...
gboolean init_local_ipv4(struct in_addr *ip) {
	...
	assert(ip != NULL);
	if (get_local_ipv4name_using_ioctl(NETDEVICE, ip))
		return TRUE;
	if (!get_local_ipv4name_using_ifconfig(NETDEVICE, nome, sizeof(nome)))
...
/*
 The ioctl SIOCGIFADDR works fine for v4 but gives error with errno==EINVAL for v6.
 */
...
gboolean init_local_ipv6(struct in6_addr *ip) {
	...
	assert(ip != NULL);
	if (!get_local_ipv6name_using_ifconfig(NETDEVICE, nome, sizeof(nome)))
		if (gethostname(nome, 256)) {
			Log("Failed to get the machine's name\n");
			return FALSE;
		}
...
// Read data from an IPv4 UDP socket
