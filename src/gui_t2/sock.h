...
void set_local_IP(); // Sets the contents of the variables with the local IP addresses
					 // BUGS: It only works for 1 address per host
gboolean init_local_ipv4(struct in_addr *ip);  //  Gets local IPv4 address
gboolean init_local_ipv6(struct in6_addr *ip);  //  Gets local IPv6 address
gboolean is_local_ip(const char *ip_str); // Returns TRUE if 'ip_str' is a local address
void translate_local_ip(struct in6_addr *ip); // Converts "::1" to the local global address

gboolean get_IPv6(const gchar *textIP, struct in6_addr *addrv6); // Reads an IPv6 Multicast address
gboolean get_IPv4(const gchar *textIP, struct in_addr *addrv4); // Reads an IPv4 Multicast address
...
char *addr_ipv4(struct in_addr *addr);	// Returns a static string with an IPv4 address contents
char *addr_ipv6(struct in6_addr *addr);	// Returns a static string with an IPv6 address contents

// dom = SOCK_DGRAM or SOCK_STREAM
// Return: -1 - error;  >0 - socket number
int init_socket_ipv4(int dom, int port, gboolean shared); // Initializes an IPv4 socket
int init_socket_ipv6(int dom, int port, gboolean shared); // Initializes an IPv6 socket
...
// Read data from an IPv6 UDP socket
// Return the number of bytes read (<0 in case of error) and the sender's address and port
