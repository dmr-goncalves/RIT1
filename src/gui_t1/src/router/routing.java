/**
 * Redes Integradas de Telecomunicações I MIEEC 2015/2016
 *
 * routing.java
 *
...
public class routing {
    ...
    public int data_delay;         // Delay before sending DATA packets
    private int data_seq;           // DATA packet sequence number
    private MulticastDaemon mdaemon;// Multicast communication object
    private RouteEntry[] tab;
    //public RouteEntry[] tabela1;
    private HashMap<Character, Router_Info> routerlist = new HashMap<Character, Router_Info>();
    public int tab_len;
    public Router_Info Router_Info2;
    public final Integer lock = new Integer(0);
    public Date lastSend;
    
    public class Router_Info {
        
        char name;
        Entry[] entries;
        int local_TTL;     // TTL for packets ROUTE received
        int route_seq;     // ROUTE packet sequence number
        Date date;         // DATA
        
        public Router_Info(char rotername, Entry[] entries, int TTL, int ROUTEseq) {
            this.name = rotername;
            this.local_TTL = TTL;
            this.entries = entries;
            this.route_seq = ROUTEseq;
            this.date = date;
        ...
        if (win.is_SendIfChanges() && network_changed) {
            ...
            if (!timeElapsed()) {
                ...
                try {
                    ...
                    neig.refresh_table(tabela);
                    
                } catch (Exception e) {
                    Log("Error Network Changed");
                }
        ...
        try {
            if (use_multicast) {
                ...
                for (int i = 0; i < vec.length; i++) {
                    ...
                    neighbour pt = neig.locate_neig(vec[i].dest);
                    if (pt != null) {
                        Iterator<Router_Info> it = routerlist.values().iterator();
                        while (it.hasNext()) {
                            Router_Info ri = it.next();
                            if ((ri != null)) {
                                DatagramPacket dp2 = make_ROUTE_packet(ri.name, ri.route_seq, ri.local_TTL, ri.entries);
                                pt.send_packet(ds, dp2);
                            }
                        }
    ...
    public boolean process_ROUTE(char sender, DatagramPacket dp,
        ...
        try {
            ...
            Router_Info2 = new Router_Info(sender, data, TTL, seq);
            routerlist.put(sender, Router_Info2);
    ...
    private boolean run_dijkstra() {
        dijkstra dij = new dijkstra(win, local_name);
        //Adds local vector
        dij.add_router(local_name, null, neig.local_vec(true));
        //Adds remote vectors
        Iterator<Router_Info> it = routerlist.values().iterator();
        while (it.hasNext()) {
            Router_Info pt = it.next();
            if ((pt != null)) {
                //if neighbour includes a pointer to the descriptor
                dij.add_router(pt.name, neig.locate_neig(pt.name), pt.entries);
            }
        }
        
        dij.add_all_vector_names();
        
        if (dij.run_dijkstra_algorithm(local_name)) {
            tab = dij.tab;
            tab_len = tab.length;
            return true;
        }
    ...
    private void timer_start(int periodo /*ms*/) {
        timer.scheduleAtFixedRate(new TimerTask() {
            
            public void run() {
                
                send_local_ROUTE(win.is_multicast());
                update_routing_table();
            }
    ...
    private void start_announce_timer() {
        //Log("start_announce_timer not implemented yet\n");
        //Log("This is the first place to start the work!\n");
        timer_start(period);
    }
    
    private void stop_announce_timer() {
        //Log("stop_announce_timer not implemented yet\n");
        timer.cancel();
    }
    ...
    public boolean timeElapsed() {
        return (lastSend == null) || ((System.currentTimeMillis() - lastSend.getTime()) >= min_interval);
    }
    ...
    public char next_Hop(char dest) {
        ...
        //duarte e bileu nao tenho a certesa s � isto
